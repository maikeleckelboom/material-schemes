 ...
 */
export function getBestContrastColor(baseColor: Color, minContrastRatio: number = 4.5): number {
  const baseTone = getLstarFromColor(baseColor);
  const whiteTone = 100;
  const blackTone = 0;
 ...
 */
export function lightenColor(color: Color, ratio: number = 1.0): number {
 ...
 */
export function darkenColor(color: Color, ratio: number = 1.0): number {
  const argb = toArgb(color);
  const {hue, chroma, tone} = Hct.fromInt(argb);
